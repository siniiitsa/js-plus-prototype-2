...
| Layout 2 | [`grunge/layout-2.md`](./grunge/layout-2.md) | `grunge-layout-2`, forked from `grunge-layout-1` (in progress since 2026-09-22; sections 1–10 — the header, the bio, the media player, the repertoire, the gallery, the pricing, the booking calendar, the events map, the enquiry form and the testimonials — done; the end-of-pass sweep is next) | Lime layout 2's ten compositions in Static Youth: each Lime layout-2 block widened to `(s.lime || s.grunge)` with the Grunge deltas inside it, all three widths per session, plus the end-of-pass sweep. Written as deltas against Lime layout 2 and Grunge layout 1. Card 2 of the setup modal becomes a fitted page; the footer is layout 1's. |
