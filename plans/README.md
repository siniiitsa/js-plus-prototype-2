...
| Layout 3 QA fixes | [`lime/layout-3-qa-fixes.md`](./lime/layout-3-qa-fixes.md) | `lime-layout-3-qa-fixes`, forked from `main` (planned, not started) | Not a fit: the tester's layout-3 reports (JP-048 blank packages, JP-049 the unchecked enquiry address, JP-050 the empty Title, JP-051 a blank form field), one entry per session, plus the end-of-pass sweep. None is inside an `s.lime` block, so every template moves. |
