...
| *A seam's binding can lie — trust the render* — a torn seam's paint can be bound to a token that resolves red and still fill and render black; the seam is the neighbour's ground | grunge/layout-4, *Grunge's layout-4 mode* ("The media foot's binding lies"); first met in grunge/layout-1, *Settled in section 3* | layout 1's media foot tear; L4: media (the 1440 foot, `sem/tag/2/bg`), repertoire (the 1440 foot, the same token) |
