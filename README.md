...
- **Imports, never fetches.** §8.6 forbids a network request in the render path.
  `vite-plugin-singlefile` forces `assetsInlineLimit = () => true`, so all twenty-five files are
  base64-inlined and the committed `index.html` still opens from `file://`. It is ~3.8 MB.
...
- **Retro and Lime are designed; Grunge, Editorial and Pop are not.** Retro ships six
  ...
  cards lay out whole Lime pages. At layout 2 the video section is not fitted (it is being
  removed), tags and audio have no frame and stay generic, and the footer is layout 1's. The
